...
def binvec2poscar(binvec, poscar, elements, alat, nsites, spc_size, unit_site):
    system = {}
    ...
    for a in range(int(system['all_atom'])):
        ...
        x, y, z = idx2coord(idx=j, site_positions=unit_site, supercell=spc_size)
...
def plotQuaternaryDiagrams(df, path_of_out):
    label = df['energy']
    phase = df.iloc[:, 1:] * 100
